...
本实现按可见索引生成数据对象，不保留完整对象数组；不定高度以预估值为基线，只在稀疏 Fenwick Tree 中保存已测量项目的高度差。因此 DOM 和数据对象内存与总条数无关，高度模型内存取决于用户实际浏览并测量过的项目数，而不是逻辑总条数。
...
简历不要堆砌 `ResizeObserver`、Fenwick Tree、`requestAnimationFrame` 等技术名词，建议按“业务问题 + 核心方案 + 可验证结果”组织。只写自己真正实现并能在面试中解释清楚的内容。

### 项目经历版

适合该方案已经在真实业务中落地，并且有优化前后的测量数据：

> 负责长列表性能优化，针对大数据量和不定高列表设计虚拟滚动方案，仅渲染视口及缓冲区节点；使用 ResizeObserver 获取真实高度、Fenwick Tree 维护累计偏移，将高度更新和滚动定位控制在 O(log n)，并通过 requestAnimationFrame 合并高频滚动更新，使 DOM 节点由 **[优化前数量]** 降至 **[优化后数量]**，滚动帧率由 **[优化前 FPS]** 提升至 **[优化后 FPS]**，长任务耗时降低 **[比例]**。

如果还处理了百万级滚动边界，可以再补一条：

> 针对浏览器单元素最大布局高度限制，设计物理滚动坐标与逻辑滚动坐标的压缩映射，支持 **[数据规模]** 条数据完整定位至首尾，并通过滚动锚点补偿解决异步测高导致的内容跳动和底部抖动问题。

### 个人项目或技术 Demo 版

如果这是个人研究或演示项目，不要写成线上业务收益，可以强调设计、实现和压测结果：

> 独立实现 Vue 3 长列表虚拟滚动方案，覆盖固定高度与不定高度场景；通过 ResizeObserver、Fenwick Tree 和动态 overscan 完成 O(log n) 的高度修正与索引定位，并使用 rAF 合帧控制滚动更新频率。

> 研究并解决浏览器最大元素高度限制，通过按需数据生成、稀疏高度索引及物理/逻辑滚动坐标映射，支持最高 10 亿条模拟数据的首尾定位，补充滚动锚定、状态持久化和边界稳定性验证。

### 精简成一条

适合简历版面有限时使用：

> 设计并实现 Vue 3 不定高虚拟列表，使用 ResizeObserver + Fenwick Tree 将高度更新和滚动定位优化至 O(log n)，结合 rAF 合帧与滚动坐标压缩，控制可见 DOM 数量并支持百万级模拟数据稳定滚动。

### 可量化指标怎么取

不要凭感觉写“性能提升 80%”，应在相同设备、浏览器、数据规模和操作路径下记录优化前后数据。可以选择以下指标：

-   DOM 节点数量：从多少个降到稳定的多少个。
-   滚动帧率：快速滚动时的平均 FPS 或低帧占比。
-   Long Task：单次最长耗时、数量或总耗时变化。
-   脚本与渲染耗时：Chrome Performance 中 scripting、rendering、painting 的变化。
-   内存：往返滚动后的堆内存和 DOM 引用是否稳定。
-   正确性：10 万、50 万、100 万条数据是否均可到达最后一项，底部是否抖动。

### 面试展开顺序

面试官追问时，可以按以下顺序展开：

1. 先说明问题：全量 DOM、滚动高频更新、不定高测量和浏览器高度上限。
2. 再说明选择：固定高度用 O(1) 计算，不定高度使用预估、实测和 Fenwick Tree。
3. 接着说明关键边界：滚动锚点、异步图片、组件状态、坐标压缩和底部抖动。
4. 最后给出证据：Performance、Memory、DOM 数量和不同数据规模下的验证结果。

简历中写“百万级”时要明确它是“百万级模拟数据压测”还是“百万级线上数据”。虚拟列表只控制渲染节点数，不代表接口传输和内存占用也已经解决；如果线上方案还使用了服务端分页、搜索或游标加载，应一并说明。
