<template>
    <main class="dynamic-list-page">
        <header class="page-header">
            ...
            <div class="metrics">
                <span><strong>{{ itemCount.toLocaleString() }}</strong> 条数据</span>
        ...
        <section class="toolbar" aria-label="列表工具栏">
            ...
            <span class="selection-status">已选择 {{ selectedIds.size }} 条</span>
            ...
            <div class="list-spacer" :style="{ height: `${physicalTotalHeight}px` }">
                ...
                >
                    <article
                        ...
                        class="list-row"
                        :class="{ selected: selectedIds.has(entry.item.id) }"
                    >
                        ...
                        <button
                            type="button"
                            class="row-select"
                            :aria-pressed="selectedIds.has(entry.item.id)"
                            @click="toggleSelected(entry.item.id)"
                        >
                            {{ selectedIds.has(entry.item.id) ? '已选择' : '选择' }}
                        </button>
                    </article>
                </div>
            </div>
...
import { createDynamicItem, DEFAULT_DATA_SIZE } from '../shared/demo-data';
// 未测量项目使用的统一预估高度。
...
// 总数据量只保存为数字，亿级数据也不会创建完整对象数组。
const itemCount = ref(DEFAULT_DATA_SIZE);
// 选择状态独立于虚拟 DOM；行滚出视口后，对应 id 仍保留在集合中。
const selectedIds = ref(new Set<number>());

// 不定高核心模型独立维护每项高度和累计高度树。
let virtualizer = new DynamicHeightVirtualizer(itemCount.value, ESTIMATED_ITEM_HEIGHT);
// 滚动容器 DOM 引用。
...
const renderedItems = computed(() =>
    // 只为当前虚拟区间生成轻量对象，内存开销与总数据量无关。
    Array.from({ length: range.value.endIndex - range.value.startIndex }, (_, offset) => {
        const index = range.value.startIndex + offset;
        return { item: createDynamicItem(index), index };
    })
);
